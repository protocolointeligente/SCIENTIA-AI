...
export async function searchCrossref(input: SearchInput): Promise<UnifiedPaper[]> {
  ...
  const mailto = process.env.CROSSREF_MAILTO ?? 'contact@scientia.ai';
  const ua = process.env.CROSSREF_USER_AGENT ?? `ScientiaAI/1.0 (mailto:${mailto})`;
